...
function drawAtom(x, y, r, state){
  // state: 'parent' | 'daughter' | {partial: frac, mode:'daughter'}
  ctx.save();
  ctx.translate(x, y);
  ...
  if (state === 'parent' || state === 'daughter'){
    ...
    // Clip a wedge for daughter portion (left-to-right)
    ctx.shadowBlur = 0;
    ctx.beginPath();
...
function draw(){
  ...
  const totalD = getTotalDaughters();
  const fullD = Math.floor(totalD);
  const fracD = totalD - fullD;

  // Determine which indices are daughters based on randomized overallOrder.
  // Important: overallOrder is built so contamination indices come first.
  const daughterSet = new Set(overallOrder.slice(0, fullD));
  const partialIndex = fracD > 0 ? overallOrder[fullD] : null;
  ...
  for (let i = 0; i < ATOM_COUNT; i++){
    ...
    if (daughterSet.has(i)){
      drawAtom(x, y, r, 'daughter');
    } else if (partialIndex === i){
      drawAtom(x, y, r, { partial: fracD });
    } else {
      drawAtom(x, y, r, 'parent');
    }
  }
}
...
function updateUI(){
  ...
  // Primary KPI: show true age if advanced mode, else apparent.
...
function toggleAdvanced(){
  ...
  // Rebuild orders so contamination occupies random positions (and decay order is disjoint).
  buildOrders();
  setSliderMinForD0();
  updateUI();
}
...
function resetAll(){
  // Keep current isotope system + half-life settings, but reset sliders.
...
function newRun(){
  // Keep settings, just randomize which atoms will be daughters as the slider increases.
  buildOrders();
  updateUI();
}

function applyPreset(p){
  // Put half-life into numeric + unit fields with a reasonable scale.
  const y = p.t12;
  // Choose the preset unit if provided; else pick a nice scale.
  let unit = p.unit || 'yr';
  let value = y / unitFactor(unit);

  // If value is awkward, auto-scale
  if (value >= 10000 && unit === 'yr'){
    unit = 'kyr'; value = y / unitFactor(unit);
  }
  if (value >= 10000 && unit === 'kyr'){
    unit = 'Myr'; value = y / unitFactor(unit);
  }
  if (value >= 10000 && unit === 'Myr'){
    unit = 'Gyr'; value = y / unitFactor(unit);
  }
...
function init(){
  ...
  initialDaughter.addEventListener('input', () => {
    // D0 changed => rebuild orders and clamp slider minimum
    buildOrders();
    setSliderMinForD0();
    updateUI();
  });
